...
function node_function()
    ...
    if place ~= "" then
        ...
        elseif place == "archipelago" then 
            print("!!!!!!!!!!!!!!!! FOUND ARCHIPELAGO: " .. Find("name"))
            mz=4 
            rank=1
        end
        ...
        if place == "archipelago" then
            if Find("name:vi") ~= "" then Attribute("name:vi", Find("name:vi")) end
            if Find("name:latin") ~= "" then Attribute("name:en", Find("name:latin")) end
        end
...
poiTags         = { aerialway = Set { "station" },
                    ...
                    historic = Set { "monument", "memorial", "castle", "ruins" },
...
poiClassRanks   = { hospital=1, monument=1, railway=2, bus=3, attraction=4, harbor=5, college=6,
...
function GetPOIRank()
    ...
    local historic = Find("historic")
    if historic ~= "" and poiTags["historic"][historic] then
        v = historic
        class = poiClasses[v] or "monument"
        rank = poiClassRanks[class] or 1
        return rank, class, v
    end

    if amenity ~= "" and poiTags["amenity"][amenity] then
        v = amenity
        class = poiClasses[v] or "amenity"
    ...
    for k, list in pairs(poiTags) do
        ...
        if list[tagVal] then
            ...
            -- Xử lý subclass đặc biệt như trong code gốc của bạn
